...
`cli_fcb.asm`, `adm3a.asm`, `savemem.asm`, `bios_disk.asm`, `sectran.asm`, `fcb_io.asm`, `mem_top.asm` and `text_ops.asm` are assembled at
...
whole group - 111 checks - when they are not on `PATH`:
```bash
...
2. The drive mapping group needs an assembler. It takes `um80` and nothing
   else, so on a machine without it 111 checks skip - more than half the
   ...
   without it still skips them, and committing those seventeen `.com` files as
